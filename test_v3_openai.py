#!/usr/bin/env python3
"""
AI Trading Strategy V3 - OpenAI GPT-4o-mini
===========================================
STABLE VERSION with OpenAI API (best reliability)

PROVIDER: OpenAI
MODEL: gpt-4o-mini (fast, cheap, stable)
COST: $0.150/1M input, $0.600/1M output = ~$1.50 for full test
UPTIME: 99.9%+
"""
...
"""
    ...
    def get_ai_signal(self, df, idx, current_position_pct):
        """Get AI trading signal via OpenAI"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                # No delay needed - OpenAI is stable
    ...
    def backtest(self, data_file):
        ...
        print(f"\n{'='*80}")
        print(f"AI TRADING STRATEGY V3 - OpenAI GPT-4o-mini")
        print(f"{'='*80}")
        ...
        print(f"\n{'='*80}")
        print(f"BACKTEST RESULTS (V3 - OpenAI)")
        print(f"{'='*80}")
        ...
        filename = f"backtest_ai_v3_openai_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
