...
class ParameterOptimizer:
    ...
    def grid_search(self, param_grid: Dict[str, List], 
                   ...
                   max_combinations: Optional[int] = None) -> pd.DataFrame:
        """
        ...
        logger.info(f"Grid search complete. Best {self.metric}: "
                   f"{df_results.iloc[0][self.metric]:.4f}")
        
        return df_results
    
    def random_search(self, param_ranges: Dict[str, Tuple], 
                     n_iterations: int = 100,
                     backtest_config: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
        """
        ...
        logger.info(f"Random search complete. Best {self.metric}: "
                   f"{df_results.iloc[0][self.metric]:.4f}")
        
        return df_results
    
    def walk_forward(self, param_grid: Dict[str, List],
        ...
        """
        logger.info(f"Starting walk-forward validation: train={train_size}, "
        ...
        if train_size + test_size > total_bars:
            raise ValueError(f"train_size + test_size ({train_size + test_size}) "
        ...
        for split in range(n_splits):
            ...
            logger.info(f"Split {split+1}/{n_splits}: train=[{start_idx}:{train_end}], "
        ...
        logger.info(f"Walk-forward complete. Avg test {self.metric}: "
                   f"{summary['avg_test_metric']:.4f} ± {summary['std_test_metric']:.4f}")
        
        return summary
    
    def _test_parameters(self, params: Dict[str, Any],
